        ...
        /*Auto.findByIdAndUpdate(autoId,req.body,function(err,autoUpdate){
            if(err){
                ...
                Auto.findById(autoId,function(err,autoNuevo){
                    //buscamor por el id y retornamos el registro viejo al nuevo
...
function deleteAuto(req, res){
    ...
    if(!idValido){
        ...
        //buscaremos un docuemnto por el id proporcionado
        Auto.findById(autoId,function(err,auto){
            if(err){
                console.log(err)
                ...
                if(!auto){
                    res.status(404).send({message:'no existe el auto con el id roporcionado'});
                }else{
                    auto.remove(function(err){
                        if(err){
                            res.status(500).send({message:'error al eliminar el auto',err:err});
                        }else{
                            res.status(200).send({message:'el auto se ha elimnado correctamente'});
                        }
                    })
                }
            }
